    ...
    Object.assign(proto, {
        ...
        async _characterOpenTagGroupEditor(groupId, category) {
            ...
            dialog.querySelector('#btn-save').onclick = async () => {
                ...
                try {
                    if (isEditing) {
                        await this.api.album.put(`/character/tag_groups/${groupId}`, { name, tags, negative_tags, animation_presets: selectedAnimPresets });
                    } else {
                        await this.api.album.post('/character/tag_groups', { name, category, tags, negative_tags, animation_presets: selectedAnimPresets });
                    }
                    this.state.character.tagGroups = await this.api.album.get('/character/tag_groups');
                    await refreshAfterTagGroupMutation();
                    close({ changed: true });
