...
def build_npcs_from_spawns(map_data, seed=None, core_api=None, personality_profile=None) -> Dict[int, dict]:
    """
    ...
    for spawn in map_data.get("npcSpawns", []):
        ...
        npc = {
            ...
            # Social pair (active interaction)
            "social_pair": None,
            
            # Energy
            "energy": rng.uniform(0.5, 1.0),
            
            # Reputation (affected by rumors)
            "reputation": 0.5,
    ...
    for other_id_str, value in relationships.items():
        if isinstance(value, (int, float)):
            ...
            migrated_relationships[other_id_str] = {
                "trust": float(value) * 0.8,
                "respect": float(value) * 0.6,
                "attraction": float(value) * 0.3,
                ...
                "interaction_count": 1,
                "positive_interactions": 1 if value > 0.3 else 0,
...
def _infer_relationship_type(value: float) -> str:
    """Infer relationship type from old single value."""
    if value <= 0.1:
        return "enemy"
    elif value <= 0.25:
        return "rival"
